...
<body>
    <div class="container">
        ...
        if (isset($_POST['sub_mit'])) {
            ...
            update_bantin($id, $ten_sp, $gia_sp, $dia_chi, $ten_hinh, $mo_ta, $date, $sdt_sp,  $rong_sp, $dai_sp, $user);
            update_bantin_chitiet($id, $so_phong, $phap_ly, $huong_nha, $dien_tich, $noi_that, $ban_do);
            echo "<script>alert('Cập nhật thành công');</script>";
        }
        if (isset($_SESSION['user'])) {
            $user = $_SESSION['user'];
            $id = $_GET['id'];
            $row = ban_tin_tendangnhap_id_sanpham($user, $id);
            foreach ($row as $a) {
        ?>
        ...
        <a name="" id="" class="btn btn-success btn-block" href="<?= $taikhoan ?>/layout.php?ban_tin_list" role="button">Danh Sách</a>
    </div>
